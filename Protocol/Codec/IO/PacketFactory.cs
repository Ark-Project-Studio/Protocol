...
namespace Protocol.Codec.IO
{
	public static class PacketFactory
	{
		private static readonly Lazy<IReadOnlyDictionary<int, Func<Packet>>> McbePacketFactories = new(BuildMcbePacketFactories);

		private static IReadOnlyDictionary<int, Func<Packet>> BuildMcbePacketFactories()
		{
			return typeof(PacketFactory).Assembly.GetTypes()
				.Where(type => !type.IsAbstract && type.Namespace == typeof(McbeLogin).Namespace)
				.Where(type => typeof(Packet).IsAssignableFrom(type) && type != typeof(UnknownPacket))
				.Where(type => type.GetConstructor(Type.EmptyTypes) != null)
				.Select(type => new { Type = type, Packet = (Packet)Activator.CreateInstance(type)! })
				.Where(entry => entry.Packet.IsMcbe)
				.GroupBy(entry => entry.Packet.Id)
				.ToDictionary(
					group => group.Key,
					group => CreateFactory(group.OrderBy(entry => entry.Type.FullName, StringComparer.Ordinal).First().Type));
		}

		private static Func<Packet> CreateFactory(Type packetType)
		{
